...
class TBInvestigationREST < Test::Unit::TestCase
  ...
  def test_00_pre_get_user_from_userservice
    guesturi = OpenTox::RestClientWrapper.get("#{$user_service}/toxbank/user?username=guest", nil, {:Accept => "text/uri-list", :subjectid => $pi[:subjectid]}).sub("\n","")
    assert_equal "#{$user_service}/toxbank/user/U2", guesturi
  end
  ...
  def test_02d_check_for_users_investigations
    result = OpenTox::RestClientWrapper.get("#{$investigation[:uri]}", {}, {:user => "#{$user_service}/toxbank/user/U271", :accept => "application/rdf+xml", :subjectid => $pi[:subjectid]}).split("\n")
    assert_match /#{@@uri}/, result.to_s
  end
  ...
  def test_02e_check_with_inexisting_user
    result = OpenTox::RestClientWrapper.get("#{$investigation[:uri]}", {}, {:user => "#{$user_service}/toxbank/user/U01", :accept => "application/rdf+xml", :subjectid => $pi[:subjectid]}).split("\n")
  ...
  def test_02f_check_for_guestuser_uris
    result = OpenTox::RestClientWrapper.get("#{$investigation[:uri]}", {}, {:user => "#{$user_service}/toxbank/user/U2", :accept => "application/rdf+xml", :subjectid => @@subjectid}).split("\n")
    assert_not_match /#{@@uri}/, result.to_s
  end
  ...
  def test_03b_put_published
    ...
    @g.query(:predicate => RDF::TB.isPublished){|r| assert_match r[2].to_s, /false/}
    response = OpenTox::RestClientWrapper.put @@uri.to_s, { :published => "true", :allowReadByGroup => "#{$user_service}/toxbank/project/G2"},{ :subjectid => $pi[:subjectid] }
    task_uri = response.chomp
    task = OpenTox::Task.new task_uri
    task.wait
    uri = task.resultURI
  ...
  def test_10_h_update_guest_policy
    response = OpenTox::RestClientWrapper.put @@uri.to_s, {:allowReadByUser => "#{$user_service}/toxbank/user/U2"},{:subjectid => $pi[:subjectid]}
    task_uri = response.chomp
  ...
  def test_20_a_post_data
    ...
    response = OpenTox::RestClientWrapper.get $investigation[:uri], {}, {:user => "#{$user_service}/toxbank/user/U479", :accept => "application/rdf+xml", :subjectid => $pi[:subjectid]}
