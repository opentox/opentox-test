...
class CompoundTest < Test::Unit::TestCase
  ...
  def test_0_compound_from_smiles
    ...
    assert_equal "[Na+].F[B-](F)(F)F", c.smiles, "A failure here might be caused by a compound webservice running on 64bit architectures. This is a known bug in OpenBabel which drops positive charges on 64bit machiens.. The only known workaround is to install the compound webservice on a 32bit machine." # still does not work on 64bit machines
  end
